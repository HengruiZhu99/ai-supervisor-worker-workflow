...
class HandoffTests(unittest.TestCase):
    def test_export_is_identity_git_contract_and_revision_bound(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run = lifecycle.start(mode="solo", objective="fix norm", acceptance_ids=("AC-1",))
    ...
    def test_cross_project_handoff_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lifecycle = RunLifecycle(first_context, runtime_env={"XDG_RUNTIME_DIR": str(Path(tmp) / "runtime")})
            run = lifecycle.start(mode="solo", objective="bounded task")
            exported = lifecycle.handoff(run["run_id"], expected_revision=run["state_revision"])
            with self.assertRaises(HandoffError):
                verify_handoff(Path(exported["handoff_path"]), resolve_project(explicit_root=second))
