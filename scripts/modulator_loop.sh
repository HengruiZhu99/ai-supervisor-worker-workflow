...
MODULATOR_MODEL="${MODULATOR_MODEL:-gpt-5.6-sol}"
...
echo "warning: scripts/modulator_loop.sh is deprecated; watchdog is deterministic" >&2
export AIFLOW_COMPAT_MODEL="$MODULATOR_MODEL"

CLI=(aiflow)
if [[ -x "$TOOL_ROOT/bin/aiflow" ]]; then CLI=("$TOOL_ROOT/bin/aiflow"); fi
exec "${CLI[@]}" --project-root "$ROOT" controller run \
  --mode orchestrated --parent-sandbox workspace-write --compat-role modulator \
