...
ACTIONABLE = {
    "process_death", "lease_expiry", "stale_heartbeat", "transaction_recovery",
    "low_disk", "low_inodes", "no_progress", "timeout",
}


class DeterministicWatchdog:
    def __init__(self, *, diagnose: Callable[[dict[str, object]], Any] | None = None) -> None:
        self.diagnose = diagnose
        self._seen: set[str] = set()

    @staticmethod
    def signature(event: Mapping[str, object]) -> str:
        stable = json.dumps(dict(event), sort_keys=True, separators=(",", ":"), default=str)
    ...
    @staticmethod
    def resource_preflight(path: Path, *, minimum_bytes: int = 50_000_000) -> list[dict[str, object]]:
        ...
        if stat.f_favail < 100:
            events.append({"kind": "low_inodes", "path": str(path), "free": stat.f_favail})
        return events
